...
# === Load Data ===
returns_df = pd.read_csv("MVO_Cumulative_Returns_.csv", index_col=0, parse_dates=True)
metrics_df = pd.read_csv("MVO_Final_Metrics_Table_.csv", index_col=0)
prices_df = pd.read_csv("MVO_Prices_.csv", index_col=0, parse_dates=True)
gamma_returns_df = pd.read_csv("MVO_Gamma_Sweep_Returns.csv", index_col=0, parse_dates=True)
variance_df = pd.read_csv("MVO_Variance_Concentration.csv", index_col=0)  # << updated filename here

returns_matrix = prices_df.pct_change().dropna()
available_metrics = list(metrics_df.index)
available_gamma_curves = list(gamma_returns_df.columns)
...
app_ui = ui.page_fluid(
    ui.navset_tab(
        ...
        ui.nav_panel(
            "📉 RMT Gamma Sweep",
            ui.input_checkbox_group(
                "selected_gamma_curves",
                "Select RMT Gamma Curves to Plot",
                choices=available_gamma_curves,
                selected=[available_gamma_curves[0]]
            ),
            ui.output_plot("gamma_sweep_plot", height="500px")
        ),
        ui.nav_panel(
            "🧠 Variance Diagnostics",
            ui.output_data_frame("variance_table")
        )
    )
)
...
def server(input, output, session):
    ...
    def gamma_sweep_plot():
        selected = input.selected_gamma_curves()
        df = gamma_returns_df.copy()
        ...
        plt.title("Cumulative Returns: RMT Gamma Sweep Models")
        plt.xlabel("Date")
        plt.ylabel("Growth of $1")
        plt.legend()
        plt.grid(True)
